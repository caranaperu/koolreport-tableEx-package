    ...
    KoolPHPTable.prototype = {
        ...
        initEvents:function()
        {
            $('#'+this.name+" tr").on('click',function(event){
                var level = $(event.currentTarget).attr("lvl");
                // Is no total row, then send event.
                if (!level) {
           ...
           /* $('#'+this.name+" tr").on('dblclick',function(event){
                event.preventDefault();
                var level = $(event.currentTarget).attr("glevel");
                $(event.currentTarget).nextUntil(':not([glevel=2])').toggle();
            }.bind(this));*/
        },
